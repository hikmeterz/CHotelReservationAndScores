...
int main(){
    ...
    scanf("%c",&temp);//clean to buffer
    printf("Please enter old scores:");//piazzada soylenen sekilde.
    fgets(old_scores,1000,stdin);
    
    printf("Please enter new scores:");//piazzada soylenen sekilde.
    fgets(new_scores,1000,stdin);
    ...
    char sayin[100];
    int sayi_indexn=0;
    int array_new_index=0;
    for(int h1=0;h1<lengthnew;h1++){
        if (new_scores[h1+1]=='\0'){
            sayin[sayi_indexn]=new_scores[h1];
            sayin[sayi_indexn+1]='\0';
            
            int deger=atoi(sayin);
            new_scores_in[array_new_index]=deger;
        
            break;
        }else if (new_scores[h1]!=' '){
            sayin[sayi_indexn]=new_scores[h1];
            sayi_indexn++;
        }else if(new_scores[h1]==' '){
            sayin[sayi_indexn]='\0';
    ...
    //dosyaya yazdirma.
    if ((fp=fopen("updated_scores_q3.txt", "w+")) == NULL) {
    ...
    for(int ii=0;ii<skor_sayisi+1;ii++){
        for(int j=0;j<id-1;j++){
            if (players[j].point==old_scores_in[ii])
            {
                ...
                players[j].point=new_scores_in[ii];
            }
